...
class PostDao extends Dao
{
    ...
    public function getPostByUserId(int $userId): array
    {
        ...
        foreach ($posts as $p) {
            $post = new Post();
            DateTime:
            $timePosted = new \DateTime($p[2]);
            $post->post($p[0], $p[1], $timePosted, $p[3], $p[4], $p[5]);
    ...
    public function getPost(\DateTime $dateTime, int $max, bool $firstLoop): array
    {
        $query = "";
        //if ($firstLoop) {
            $query = "SELECT * from post where dateTime>=:dateTime order by dateTime DESC limit " . $max;
       // } else {
          // $query = "SELECT * from post where dateTime>:dateTime order by dateTime DESC limit " . $max;
       //}
        $statement = $this->getConn()->prepare($query);
    ...
    public function getOlderPost(\DateTime $dateTime, int $max): array
    {
        $query = "SELECT * from post where dateTime<:dateTime order by dateTime DESC limit " . $max;
        $statement = $this->getConn()->prepare($query);
        $dateTime = $dateTime->format('Y-m-d H:i:s');
